...
public class TeacherManager {
    ...
    public void hireTeacherWithException(ProfessorZ teacher) throws ProfessorIsAlreadyHiredException {
        if (professors.contains(teacher)){
            throw new ProfessorIsAlreadyHiredException("Professor " + teacher.getName() +
                    " is already part of the College staff");
        }
    ...
    public void countUniqueWordsInFile(File file) throws IOException {
        ...
        FileUtils.writeLines(file, "UTF-8", List.of("There are a total of " +
                uniqueWords.size() + " unique words in this file."), true);
    }

    public void showProfessorByCategory(TeacherCategory category) {
        boolean found  = professors.toList().stream()
                .filter(e -> e.getTeacherCategory() == category)
                .peek(System.out::println)
                .findAny()
                .isPresent();

        if (!found) {
            LOGGER.info("No professors found in the {} category.", category);
        }
    }

    public void filterProfessorsBySubject(SubjectZ subject) {
        Filter<ProfessorZ> filterByCategory = professor -> professor.getSubject().equals(subject);
        professors.toList().stream()
                .filter(filterByCategory::test)
